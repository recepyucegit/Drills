...
namespace FitnessTracker
{
    public partial class MainForm : System.Windows.Forms.Form
    {
        ...
        private void SetupDataGridView()
        {
            ...
            dgvExercises.Columns["Weight"].Width = 80;

            // RPE (Rate of Perceived Exertion)
            dgvExercises.Columns.Add("RPE", "RPE (1-10)");
            dgvExercises.Columns["RPE"].Width = 70;
        ...
        private void CreateEmptyRows()
        {
            ...
            UpdateRepsColumnsVisibility();
        }

        private void UpdateRepsColumnsVisibility()
        {
            if (dgvExercises.Rows.Count == 0) return;

            // En yüksek set sayısını bul
            int maxSetCount = 1;
            foreach (DataGridViewRow row in dgvExercises.Rows)
            {
                if (int.TryParse(row.Cells["SetCount"].Value?.ToString(), out int setCount))
                {
                    if (setCount > maxSetCount) maxSetCount = setCount;
                }
            }

            // Reps sütunlarını göster/gizle
            for (int i = 1; i <= 4; i++)
            {
                dgvExercises.Columns[$"Reps{i}"].Visible = i <= maxSetCount;
            }
        }
        ...
        private void BtnCalculate_Click(object sender, EventArgs e)
        {
            ...
            for (int rowIndex = 0; rowIndex < dgvExercises.Rows.Count; rowIndex++)
            {
                ...
                // RPE Validation
                if (!int.TryParse(row.Cells["RPE"].Value?.ToString(), out int rpe) || rpe < 1 || rpe > 10)
                {
                    MessageBox.Show($"Satır {rowIndex + 1}: RPE değerini 1-10 arasında girin!", "Hata", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    hasError = true;
                    break;
                }
        ...
        private void DisplayWorkoutTable()
        {
            ...
            {
                Text = "EGZERSİZ                    MEVCUT KİLO  RPE   SETLER        EN DÜŞÜK   SONRAKİ KİLO   ARTIŞ",
            ...
            foreach (var result in exerciseResults)
            {
                ...
                string currentWeight = $"{result.CurrentWeight}kg".PadRight(10);
                string rpe = $"{result.RPE}".PadRight(5);
                ...
                {
                    Text = $"{exerciseName}  {currentWeight}  {rpe}  {sets}  {minReps}  {nextWeight}  {increaseText}",
        ...
        private void PrepareWorkoutData()
        {
            ...
            foreach (var result in exerciseResults)
            {
                ...
                currentWorkoutData += $"RPE: {result.RPE}/10\n"; // <-- EKLENDİ
    ...
    public class ExerciseResult
    {
        ...
        public int RPE { get; set; } // <-- EKLENDİ
